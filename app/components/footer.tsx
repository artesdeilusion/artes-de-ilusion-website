"use client"
import { GitHub, Instagram, LinkedIn } from "@mui/icons-material";
 import Link from "next/link";
 

export default function Footer() {
 
    return (
        <section className="border-t text-black" >
            <div className=" px-4 pt-16 mx-auto  lg:max-w-screen-xl   lg:px-8">
                <div className="grid gap-16 row-gap-10 mb-8 lg:grid-cols-6">

                    <div className="grid   gap-5 row-gap-8 lg:col-span-4 md:grid-cols-4">
                        <div>
                            <p className=" tracking-wide 800">
Service
                            </p>
                            <ul className="mt-2 text-xs space-y-2">
                                <li>
                                    <Link
                                        href="/service/our-works"
                                        className="transition duration-200 ease-in-out  underline-animation"
                                    >
Portfolio

                                    </Link>
                                </li>
                               
                            </ul>
                        </div>
                        <div>
                            <p className=" tracking-wide 800">
Şirket
                            </p>
                            <ul className="mt-2 text-xs space-y-2">
                                <li>
                                    <Link
                            href="https://www.linkedin.com/company/artes-de-ilusion"target="_blank"
                            className="transition duration-200 ease-in-out  underline-animation"
                                    >
                                        Kariyer
                                    </Link>
                                </li>
                                <li>
                                    <Link
                                        href="/company"
                                        className="transition duration-200 ease-in-out  underline-animation"
                                    >
                                        Hakkımızda
                                    </Link>
                                </li>
                                <li>
                                    <Link
                                        href="/contact"
                                        className="transition duration-200 ease-in-out  underline-animation"
                                    >
                                        İletişim
                                    </Link>
                                </li>
                            </ul>
                        </div>


                        <div>
                            <p className=" tracking-wide 800">
                                Sektörel

                            </p>
                            <ul className="mt-2 text-xs space-y-2">
                                <li>
                                    <Link
                                        href="/commercial"
                                        className="transition duration-200 ease-in-out  underline-animation"
                                    >
                                        Commercial

                                    </Link>
                                </li>  <li>
                                    <Link
                                        href="/indie"
                                        className="transition duration-200 ease-in-out  underline-animation"
                                    >
                                        Indie

                                    </Link>
                                </li>  <li>
                                    <Link
                                        href="/music"
                                        className="transition duration-200 ease-in-out  underline-animation"
                                    >
                                        Müzik

                                    </Link>
                                </li>

                            </ul>
                        </div>

                        <div>
                            <p className=" tracking-wide 800">
İletişim              
  </p>
                            <ul className="mt-2 text-xs space-y-2">
                                <li>
                                    <Link
                                        href="mailto:info@artesdeilusion.com"
                                        target="_blank"
                                        className="transition duration-200 ease-in-out  underline-animation"
                                    >
                                        info@artesdeilusion.com
                                    </Link>
                                </li>

                            </ul>
                        </div>
                    </div>
                </div>
                <div className="flex flex-col justify-between pt-5 pb-10 border-t sm:flex-row">
                    <p className="text-xs ">
                        © Copyright 2024 Artes de Ilusion. All rights reserved.

                    </p>
                    <div className="flex items-center mt-4 space-x-4 sm:mt-0">
 
                        <a
                            href="https://www.instagram.com/artesdeilusion"
                            target="_blank"
                            className=" selection:transition-colors duration-300 hover:text-red-500"
                        >
                            <Instagram></Instagram>
                        </a>
                        <a
                            href="https://www.github.com/artesdeilusion"                            target="_blank"

                            className=" selection:transition-colors duration-300 hover:text-red-500 "
                        >
                            <GitHub/>
                        </a>
                        <a
                            href="https://www.linkedin.com/company/artes-de-ilusion"                            target="_blank"

                            className=" selection:transition-colors duration-300 hover:text-red-500 "
                        >
                            <LinkedIn/>
                        </a>
                        <a
                            href="https://www.behance.net/aicom" target="_blank"

                            className=" selection:transition-colors duration-300 hover:text-red-500 "
                        >
                            <svg xmlns="http://www.w3.org/2000/svg" className="hover:text-red-500" width="24" height="24" viewBox="0 0 24 24"><path d="M22 7h-7v-2h7v2zm1.726 10c-.442 1.297-2.029 3-5.101 3-3.074 0-5.564-1.729-5.564-5.675 0-3.91 2.325-5.92 5.466-5.92 3.082 0 4.964 1.782 5.375 4.426.078.506.109 1.188.095 2.14h-8.027c.13 3.211 3.483 3.312 4.588 2.029h3.168zm-7.686-4h4.965c-.105-1.547-1.136-2.219-2.477-2.219-1.466 0-2.277.768-2.488 2.219zm-9.574 6.988h-6.466v-14.967h6.953c5.476.081 5.58 5.444 2.72 6.906 3.461 1.26 3.577 8.061-3.207 8.061zm-3.466-8.988h3.584c2.508 0 2.906-3-.312-3h-3.272v3zm3.391 3h-3.391v3.016h3.341c3.055 0 2.868-3.016.05-3.016z"/></svg>
                        </a>
                    </div>
                </div>
            </div>
        </section>
    );
};